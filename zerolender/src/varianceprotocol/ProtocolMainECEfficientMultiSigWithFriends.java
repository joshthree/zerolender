...
public class ProtocolMainECEfficientMultiSigWithFriends {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, SecurityException, InvalidStringFormatException, IOException, ClassNotFoundException, MultipleTrueProofException, NoTrueProofException, ArraySizesDoNotMatchException{
		...
		if(logging)
		{
			PrintStream err = new PrintStream(new File(args[4] + "_run_error"));
			PrintStream normal = new PrintStream(new File(args[4] + "_run_outputs"));
		...
		Scanner friendFile = new Scanner(new File(args[7]));
		...
		Writer arg0 = null;
		try {
			arg0 = new FileWriter("Verifier_Transcript_" + args[4] + "_Variance");
		} catch (IOException e) {
		...
		while((dataRow = br.readLine()) != null)
		{
			...
			if(provers.length < n)
			{
				...
				ZKPProtocol keyProtocol = VarianceToolkit.createMultiSigProof(n, k, ecSchnorr, ecSchnorr);

				provers[n-1][k-1] = VarianceToolkit.createVarianceMultiSigProof(keyProtocol, ecSchnorr);
				
				CryptoData keyEnv = VarianceToolkit.createMultiSigEnvironment(n, k, miniEnv, revMiniEnv);
...
//			System.out.println("In main:  " + environments[n-1][k-1]);
			...
			if(keys != null && keys.id == counter && keys.keys.length >= k){ 
				/**/System.out.println("I own this account");
				...
				for(int i = 0; i < n; i++)
				{
					...
					if(j < keys.positions.length && keys.positions[j] == i) {
						internalCommitment[proverIndex][i] = new ECPedersenCommitment(BigInteger.ONE, commKeys[proverIndex][i], revMiniEnv);
						j++;
					}
					else
					{
						internalCommitment[proverIndex][i] = new ECPedersenCommitment(BigInteger.ZERO, commKeys[proverIndex][i], revMiniEnv);
					}
				}
				CryptoData keyData = VarianceToolkit.createMultiSigProverData(n, k, acc[proverIndex], keys.keys, keys.positions, miniEnv, internalCommitment[proverIndex], commKeys[proverIndex], order, r);
			...
			else
			{
				...
				ECPoint comm = commitment.getCommitment(miniEnv);
				CryptoData keyData = VarianceToolkit.createMultiSigSimulatorData(n, k, acc[proverIndex], miniEnv, internalCommitment[proverIndex], commKeys[proverIndex], order, r);
			...
			if((proverIndex == blockSize || (!br.ready() && proverIndex != 0)))
			{
				...
				for(int i = 0; i < proverIndex; i++)
				{
					...
					CryptoData keyVerifierData = VarianceToolkit.createMultiSigVerifierInputs(n, k, acc[i], otherInternalComms[i], miniEnv, ecSchnorr);
...
//		if(resultOfComparison == -1) System.out.println("I have less :-(");
		...
		try {
			arg0 = new FileWriter("Output_" + args[4]);
		} catch (IOException e) {
