...
public class ProtocolMainMixed {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, SecurityException, InvalidStringFormatException, IOException, ClassNotFoundException{
	    ...
	    DLPedersenCommitment[] otherCommitments = new DLPedersenCommitment[blockSize];
		...
		ZKPData[] commitmentEnv = miniEnv.getZKPDataArray();
		...
		othersSumCommitment = new DLPedersenCommitment(BigInteger.ZERO, new BigInteger[] {BigInteger.ZERO}, commitmentEnv);
		...
		BigInteger[][] acc = new BigInteger[blockSize][2];
...
//				System.out.println("IN TRY:			" + stringData[0] + "     " + stringData[1]);
			...
			if(k[1].equals(acc[proverIndex][1])){
				...
				DLPedersenCommitment commitment = new DLPedersenCommitment(acc[proverIndex][0], new BigInteger[] {pcKey}, commitmentEnv);
				
				sumKey[0] = sumKey[0].add(pcKey).mod(orderP);
				sum = sum.add(acc[proverIndex][0]).mod(orderP);
				
				//System.out.println("Sending true commitment:  " + commitment.getCommitment());
				out.writeObject(commitment);
				
				bIntArray[0] = pcKey;
				bIntArray[1] = commitment.getCommitment().multiply(h.modPow(acc[proverIndex][0].negate(), p)).mod(p);
				bIntArray[2] = new BigInteger(bitLength - 1, r);
			...
			else
			{
				...
				bIntArray[0] = commitment.getCommitment().multiply(h.modPow(acc[proverIndex][0].negate(), p)).mod(p);
				bIntArray[1] = new BigInteger(bitLength - 1, r);
			...
			{
				runProver(prover, verifier, host, in, out, otherCommitments, r, h, p, bitLength, commitmentEnv,
	...
	private static void runProver(ZKPProverInterface prover, ZKPVerifierInterface verifier, ServerSocket host,
		...
		for(int i = 0; i < proverIndex; i++)
		{
			BigInteger comm = otherCommitments[i].getCommitment();					
			ZKPData[] dataArrayInner = new ZKPData[2];
			...
			bIntArray[0] = comm.multiply(h.modPow(acc[i][0].negate(), p)).mod(p);
...
//							System.out.println(myVerify + " " + otherVerify);
					...
					else othersSumCommitment = othersSumCommitment.multiplyCommitment(otherCommitments[i], commitmentEnv);
