...
import zero_knowledge_proofs.ECProvisionsVerifier;
...
public class ProtocolMainECMixedProvisions {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, SecurityException, InvalidStringFormatException, IOException, ClassNotFoundException{
		...
		ZKPProverInterface.registerProtocol("Provisions", ECProvisionsProver.class, true);
		ZKPProverInterface prover = ZKPProverInterface.generateProver("Provisions");

		ZKPVerifierInterface.registerProtocol("Provisions", ECProvisionsVerifier.class, true);
		...
		ZKPData environment = new ZKPDataArray(new ZKPData[] {new ECCurveData(c,g), new ECPointData(h)});
		ZKPData[] commEnv = environment.getZKPDataArray();
		
		ZKPData[] commitment1Env = new ZKPData[] {new ECCurveData(c, h), null};
		ZKPData[] commitment2Env = new ZKPData[] {new ECCurveData(c,g), new ECPointData(h)};
		ZKPData commEnv2 = new ZKPDataArray(commitment2Env);
		...
		while((dataRow = keyBr.readLine()) != null)
		{
			...
			try
			{
				...
				keyPair[1] = new ECPointData(c.createPoint(new BigInteger(decoder.decode(stringData[1])),new BigInteger(decoder.decode(stringData[2]))));
		...
		ZKPData[][] acc = new ZKPData[blockSize][3];

		ZKPData[] bIntArray = new ZKPData[13];
...
//				System.out.println("IN TRY:			" + stringData[0] + "     " + stringData[1]);
				BigInteger b = new BigInteger(stringData[0]);
				acc[proverIndex][0] = new BigIntData(b);
				acc[proverIndex][1] = new ECPointData(c.createPoint(new	BigInteger(decoder.decode(stringData[1])),new BigInteger(decoder.decode(stringData[2]))));
			...
			{
				runProver(prover, verifier, host, in, out, otherCommitments, r, h, bitLength, commEnv2,
	...
	private static void runProver(ZKPProverInterface prover, ZKPVerifierInterface verifier, ServerSocket host,
			...
			int bitLength, ZKPData commitmentEnv, ZKPData environment, int proverIndex,
		...
		{					
			...
			ZKPData otherInput = new ZKPDataArray(new ZKPData[] {acc[i][2], acc[i][1], new ECPointData(otherCommitments[i][0].getCommitment(commitmentEnv)), new ECPointData(otherCommitments[i][1].getCommitment(commitmentEnv))});
