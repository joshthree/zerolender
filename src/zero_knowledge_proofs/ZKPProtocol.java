...
//		}	
	 ...
	 * @throws NoTrueProofException 
	 */
	public boolean verifyFiatShamir(CryptoData input, CryptoData a, CryptoData z, CryptoData environment) throws IOException, ClassNotFoundException, MultipleTrueProofException, NoTrueProofException, ArraySizesDoNotMatchException {
		try {
			MessageDigest digest = MessageDigest.getInstance("SHA-256");
			byte[][] bytes = new byte[][] {a.getBytes(),environment.getBytes(),input.getBytes()};
			BigInteger c = new BigInteger(digest.digest(Arrays.concatenate(bytes))).mod(BigInteger.ONE.shiftLeft(255));
			
			return verifyResponse(input, a, z, c, environment);
		} catch (NoSuchAlgorithmException e) {
			e.printStackTrace();
			return false;
		}
