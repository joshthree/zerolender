...
public class ProverProtocolMain {
	public static void main(String[] args)
	{
		...
		String timerFile = baseName + ".part1.prover.time";
		...
		try {
			...
			transcriptStream.writeObject(new ECPointData(h));
		...
		timerFile = baseName + ".part2.prover.time";
...
//					System.out.printf("%d, %d\n", i, j);
					if(shuffledMappingTableValues[j][i]) {
						investorStream[i].writeObject(table.uncommittedTable[j]);
						investorStream[i].writeObject(table.keys[j]);
					}
				}
			} catch (IOException e) {
				// TODO Auto-generated catch block
		...
		try {
			transcriptStream.writeObject(new CryptoDataArray(mTable));
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		System.out.println("Done with repayment table.");
		//Part 3:
		//TO BE CONTINUED (probably in another section		
	}
}
