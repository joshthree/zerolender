...
//			System.out.println("Running columns " + i);
	...
	public void consolidateTable(int numInvestors, boolean[][] mappingTableValues, BigInteger[][] mappingTableKeys, ECPoint[][] mappingTable, int[] mValues, BigInteger[] mKeys, ECPoint[] mTable, SecureRandom rand, ObjectOutputStream publicOut, CryptoData environment) {
		...
		ECPoint g = e[0].getECPointData(c);
		ECPoint h = e[1].getECPointData(c);
		CryptoData revEnv = new CryptoDataArray(new CryptoData[] {new ECCurveData(c, h), new ECPointData(g)});
...
//			}
		...
		ECPoint[][][] vHat = new ECPoint[numInvestors][repayments][units];
		BigInteger[][][] vKeys = new BigInteger[numInvestors][repayments][units];
		consolidatedTable = new ECPoint[numInvestors][repayments];
		consolidatedTableValues = new int[numInvestors][repayments];
		consolidatedTableKeys = new BigInteger[numInvestors][repayments];
		for(int k = 0; k < numInvestors; k++) {
			for(int i = 0; i < units; i++){
				CryptoData[] innerPublicData0 = new CryptoData[repayments+1];
				CryptoData[] innerPrivateData0 = new CryptoData[repayments+1];
				CryptoData[] innerPublicData1 = new CryptoData[repayments+1];
				CryptoData[] innerPrivateData1 = new CryptoData[repayments+1];
				CryptoData[] middlePrivateData = new CryptoData[3];
				CryptoData[] middlePublicData = new CryptoData[2];
				
				if(mappingTableValues[i][k]) {
					for(int j = 0; j < repayments; j++) {
						...
						try {
							publicOut.writeObject(new ECPointData(vHat[k][j][i]));
						} catch (IOException e1) {
							e1.printStackTrace();
						}
					}
					innerPublicData0[repayments] = new CryptoDataArray(new CryptoData[] {new ECPointData(mappingTable[i][k])});
					innerPrivateData0[repayments] = new CryptoDataArray(new BigInteger[] {ZKToolkit.random(order, rand)});
					innerPublicData1[repayments] = new CryptoDataArray(new CryptoData[] {new ECPointData(mappingTable[i][k].subtract(g))});
					innerPrivateData1[repayments] = new CryptoDataArray(new BigInteger[] {ZKToolkit.random(order, rand), mappingTableKeys[i][k]});
					
					middlePrivateData[0] = new CryptoDataArray(innerPrivateData0);
					middlePrivateData[1] = new CryptoDataArray(innerPrivateData1);
					...
					innerPublicData0[repayments] = new CryptoDataArray(new CryptoData[] {new ECPointData(mappingTable[i][k])});
					innerPrivateData0[repayments] = new CryptoDataArray(new BigInteger[] {ZKToolkit.random(order, rand), mappingTableKeys[i][k]});
					innerPublicData1[repayments] = new CryptoDataArray(new CryptoData[] {new ECPointData(mappingTable[i][k].subtract(g))});
					innerPrivateData1[repayments] = new CryptoDataArray(new BigInteger[] {ZKToolkit.random(order, rand)});
...
//				/**/System.out.println(innerPrivateData0.length);
//				/**/System.out.println(new CryptoDataArray(middlePrivateData));
//				/**/System.out.println(new CryptoDataArray(middlePublicData));
//				/**/System.out.println(largeEnv);
				...
				try {
					...
					publicOut.writeObject(transcript);
				} catch (ClassNotFoundException | IOException | MultipleTrueProofException | NoTrueProofException
						| ArraySizesDoNotMatchException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
...
//				try {
//					if(!largeProof.verifyFiatShamir(new CryptoDataArray(middlePublicData), transcript[0], transcript[1], largeEnv)) System.out.println("Bad Proof on large proof");
//				} catch (ClassNotFoundException | IOException | MultipleTrueProofException | NoTrueProofException
//						| ArraySizesDoNotMatchException e1) {
//					// TODO Auto-generated catch block
//					e1.printStackTrace();
